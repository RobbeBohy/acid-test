...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        nargs="+",
        help="List of JSON results files to include in the report.",
    )
    parser.add_argument(
        "case",
        type=str,
...
def plan(args: argparse.Namespace):
    # Analyze the provided JSON results to decide what to include in the report.
    has_acint = True
    ...
    path_settings = "../1_dataset/settings.json"
...
def plan_scaling(
    ...
    runpy(
        f"./scripts/plot_scaling.py {kernel} {quantity} {path_settings} {path_zip} ../matplotlibrc"
...
def plan_cutoff(case: str, kernel: str, quantity: str, paths_json: list[Path]):
    ...
    runpy(
        f"./scripts/plot_cutoff.py {kernel} {quantity} {path_zip} ../matplotlibrc"
